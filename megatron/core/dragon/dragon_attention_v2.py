...
@dataclass
class SelfDiffAttentionV2Submodules:
    """
...
class DiffAttentionV2(MegatronModule, ABC):
    ...
    def forward(
        ...
        *,
        ...
        # alpha_k: [L, B, H_noise_local, 1], alpha_v: [L, B, H_noise_local, 1]
        # B_k: [L, B, r*Dk], B_v: [L, B, r*Dk]
        # lambda_proj: [L, B, H_noise_local, 1], gate: [L, B, H_signal_local, Dk]
        ...
        if packed_seq_params is not None:
            ...
            lambda_proj = lambda_proj.squeeze(1)
        ...
        if self.checkpoint_core_attention and self.training:
            ...
            if inference_context is None or inference_context.is_static_batching():
                ...
                with get_fine_grained_offloading_context(self.offload_core_attention):
                    ...
                    assert core_attn_out.size(-2) == self.num_attention_heads_per_partition
                    assert core_attn_out.size(-1) == self.hidden_size_per_attention_head
            else:
        ...
        core_attn_out = attn_sig - torch.sigmoid(lambda_proj).unsqueeze(-1) * attn_noi # (..., H_noise, snr, D)
        core_attn_out = core_attn_out.reshape(*core_attn_out.shape[:-3], -1, self.hidden_size_per_attention_head) # (..., num_signal_heads, D)
...
class SelfDiffAttentionV2(DiffAttentionV2):
    ...
    def __init__(
        ...
        # we make groups of 4 heads (q), 1 noise head (A_k, alpha_k, A_v, alpha_v, lambda), and 3 signal heads (gate).
    ...
    def get_query_key_value_tensors(self, hidden_states):
        ...
        mixed_noise_heads = mixed_in[..., accum:accum + 1*(r + alpha_dim + r + alpha_dim + 1)]; accum += 1*(r + alpha_dim + r + alpha_dim + 1)
        mixed_signal_heads = mixed_in[..., accum:accum+3*Dk]

        # mixed_heads: [L, B, H_super_local, 4*Dk]
        # mixed_noise_heads: [L, B, H_super_local, 1*(r + alpha_dim + r + alpha_dim + 1)]
        ...
        A_k = mixed_noise_heads[..., 0:0+r]; accum = r
        alpha_k = mixed_noise_heads[..., accum:accum+alpha_dim] if self.config.token_shift else None; accum += alpha_dim
        A_v = mixed_noise_heads[..., accum:accum+r]; accum += r
        alpha_v = mixed_noise_heads[..., accum:accum+alpha_dim] if self.config.token_shift else None; accum += alpha_dim
        lambda_proj = mixed_noise_heads[..., accum:accum+1]
        ...
        # q: [L, B, H_local, dk]
        # A_k: [L, B, H_noise_local, r], alpha_k: [L, B, H_noise_local, 1], A_v: [L, B, H_noise_local, r], alpha_v: [L, B, H_noise_local, 1], lamda_proj: [L, B, H_noise_local, 1]
